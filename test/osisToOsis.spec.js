...
describe("Round trips", function() {
	it ("should round-trip single books", function() {
		...
		for (const book of Object.keys(books)) {
			expect(c.toReadable(`${book}`)).toEqual(`${book}`)
			expect(c.toReadable(`${book}.2`)).toEqual(`${book}.2`)
			expect(c.toReadable(`${book}.2.3`)).toEqual(`${book}.2.3`)
	...
	it ("should round-trip multiple books", function() {
		...
		for (const test of tests) {
			expect(c.toReadable(test)).toEqual(test)
		}
		expect(c.toReadable(tests.join(","))).toEqual(tests.join(","))
	})
})