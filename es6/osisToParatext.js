...
(function () {
	...
	function partToParatext(paratext) {
		// `end` may be undefined.
		const [start, end] = paratext.split("-");
		const [startBook, startChapter, startVerse] = start.split(".");
		...
		const startParatext = getStart(startBook, startChapter, startVerse);
		// If `end` is undefined, don't go any further; we know it's not a range.
		if (end === undefined) {
			return startParatext;
		}
		const endParatext = getEnd(end, startBook, startChapter, startVerse);
	...
	function getStart(book, chapter, verse) {
		let out = osises[book];
	...
	function getEnd(end, startBook, startChapter, startVerse) {
		...
		if (endBook !== startBook) {
			...
			const out = osises[endBook] + " " + endChapter;
